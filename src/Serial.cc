...
string Serial::str() const { return iss.str(); }
...
bool Serial::empty() const { return (iss.str().length() == 0); }
...
void operator<<=(char &data, Serial &cereal) { cereal.get(data); }

void operator<<=(bool &data, Serial &cereal) { cereal.get(data); }

void operator<<=(short &data, Serial &cereal) { cereal.get(data); }

void operator<<=(int &data, Serial &cereal) { cereal.get(data); }

void operator<<=(long &data, Serial &cereal) { cereal.get(data); }
